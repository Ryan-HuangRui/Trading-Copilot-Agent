...
## Independent earnings research (P1/P2 interfaces; P3 daily wrapper)

Use `docs/contracts/earnings-research.md`, `docs/earnings-research-p1-p2-runbook.md`, `docs/earnings-research-p3-runbook.md` and the earnings workflow plan. P1/P2 expose collection, queue and manual role handoff interfaces. The P3 NAS wrapper runs once daily at 10:00 Asia/Shanghai, including weekends. Set `mute=true`; disable intermediate and final-result auto-forwarding. Role logs and normal skips stay local. Only the finalizer may explicitly call `cc-connect send` when its notification decision warrants it. Production activation and its acceptance evidence are recorded separately in NAS runtime.
