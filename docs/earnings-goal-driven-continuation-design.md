...
- starter 与直接 worker 在首次 progress 读取前持专用 migration lock 检查季度 schema；发现旧列集时先把原 SQLite 复制到 `runtime/earnings/migrations/`，记录 running/completed/failed 状态，再执行幂等加列迁移。迁移失败不得启动研究窗口。
