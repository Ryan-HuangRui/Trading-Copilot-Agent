...
Dependency recovery is exact-target and preview-first. Reusing an older completion requires exact frozen source/calculation/method/profile equality; differing legacy raw configuration hashes require matching immutable semantic-basis proofs for both tasks. A one-sided snapshot never proves compatibility. If a proof is unavailable, reuse remains forbidden; an operator may instead apply the separately audited `exclude-dependency` action. Exclusion retains attempts and the original failure in its audit, never makes an older report or its dependent frozen task current, and blocks the affected non-running quarterly DAG before a new revision is formed without that report, source or dependency. Disclosed and fetched facts remain visible, but excluded research is not counted as researched or exposed as a permitted model input. The exact event/operating-period/source lineage remains a deterministic full-eligibility gap until a later completed, registered report for that same lineage supersedes it. A running affected quarterly scope makes exclusion ineligible and must be retried after the lease/work ends.
...
Publication recovery separates batch remaining time, stage start thresholds and per-execution timeout. A completed writer is immutable and may be followed on a later day by one new audited checker attempt; an expired absolute deadline or changed invocation timeout cannot permanently poison the job. Cached failed results consume no new model allowance, retain semantic/deterministic feedback, and may schedule at most one repair writer+checker pair. Only successful results with a manifest may enter cloud state. Explicit operator recovery is exact-target and preview-first. Attempt-bearing recovery is backed up and one-shot. `reconcile-publication` is a zero-model, zero-cloud-operation exception: it is safely repeatable, requires an explicitly selected runtime config (and deployment in cloud mode), and changes quarterly publication/checker/cloud stages only when the registered manifest/hash, clean checker result, exact current synthesis, reader hashes, job scope/version and the selected deployment's verified user-route record (or explicitly configured local archive state) all match. A completed cloud stage without that exact evidence is blocked and cannot finalize the revision.
