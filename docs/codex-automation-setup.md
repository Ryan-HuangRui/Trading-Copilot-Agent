...
10. Optionally run read-only account and position review:
...
11. Run daily self-review:
   python3 script/trading_copilot.py daily-self-review --date <SNAPSHOT_DATE> --append
12. Update the Longbridge watchlist group `今日关注` as a full replacement from the post-market focus list:
...
8. Optionally run read-only account and position review:
...
9. Incrementally add the pre-market focus symbols to the Longbridge watchlist group `今日关注`:
...
- Confirm `report/<DATE>/post-market.md`, `report/<DATE>/exec-brief.md`, `report/<DATE>/pre-market.md`, and `report/<DATE>/signals.json` are generated by their respective jobs.
...
- Report validation failure: fix the generated report and `signals.json` so every actionable candidate has setup, structured trigger price, structured invalidation price, and risk framing; rerun `validate-report` before Longbridge sync.
