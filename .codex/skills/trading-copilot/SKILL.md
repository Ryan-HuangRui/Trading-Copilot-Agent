...
8. Run `python3 script/trading_copilot.py paper-strategy-review` to aggregate paper execution reviews by setup and symbol.
9. Run `python3 script/trading_copilot.py paper-order-cancel --date <DATE>` to prepare a dry-run cancel plan for expired unfilled entry orders.
10. Only when the user explicitly wants simulated cancellation, run `TRADING_COPILOT_PAPER_EXECUTION=enabled python3 script/trading_copilot.py paper-order-cancel --date <DATE> --execute`.
11. Run `python3 script/trading_copilot.py paper-protective-stop-plan --date <DATE>` to prepare a dry-run protective stop plan for filled long entries.
12. Only when the user explicitly wants simulated protective stop submission, run `TRADING_COPILOT_PAPER_EXECUTION=enabled python3 script/trading_copilot.py paper-protective-stop-plan --date <DATE> --execute`.
13. Run `python3 script/trading_copilot.py paper-take-profit-plan --date <DATE>` to prepare a dry-run TP1 partial-exit plan for filled long entries.
14. Only when the user explicitly wants simulated TP1 submission, run `TRADING_COPILOT_PAPER_EXECUTION=enabled python3 script/trading_copilot.py paper-take-profit-plan --date <DATE> --execute`.
15. Run `python3 script/trading_copilot.py paper-break-even-stop-plan --date <DATE>` to prepare a dry-run break-even stop movement plan after TP1 fill evidence exists. This workflow is plan-only and must not cancel, replace, or submit broker orders.
16. Run `python3 script/trading_copilot.py paper-trade-review --date <DATE> --session pre-market --append` only after paper executions exist and should be recorded.
17. Treat paper results as execution feedback. Do not promote paper P/L directly into `knowledge/refined/`.
