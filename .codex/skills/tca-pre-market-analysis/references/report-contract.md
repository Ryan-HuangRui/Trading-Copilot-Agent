...
- 若存在 `runtime/account/<PRE_MARKET_DATE>/plugin-account-snapshot.json`，它是 IBKR 与 Longbridge 插件只读持仓的规范化证据。必须保留券商来源，组合层按 symbol/currency 聚合，跨券商重复标的不得重复计算为两个独立风险结论。缺少价格、汇率或某一券商输入时必须明确披露，不得猜测。
...
- 逐持仓条件化建议：<HOLD_WATCH / NO_ADD / RISK_REVIEW / EXIT_IF_INVALIDATED / DATA_INSUFFICIENT>
- 维持条件：<结构和关键位条件>
- 失效/退出复核条件：<只描述触发人工复核的情景，不是卖出指令>
- 加仓约束：<仅已有浮盈、完整 setup 与风险预算允许时才可评估；亏损仓默认 NO_ADD>
- 特殊风险：<杠杆 ETF、保证金、缺失价格、未纳入今日计划>
- 边界：只读风险复核，不生成加仓、减仓、卖出或订单动作。
...
完整报告还必须包含 `## 持仓与组合风险`，逐持仓连接价格结构、关键位、计划覆盖和失效证据，并披露券商覆盖、跨账户重复暴露、集中度口径和数据缺口。该节只能输出人工复核情景，不得输出确定性仓位调整指令。
每个持仓必须给出一个条件化建议标签、维持条件、失效/退出复核条件和加仓约束。已有亏损仓默认 `NO_ADD`；只有已有浮盈、价格结构确认、完整 setup 与组合风险预算同时支持时，才允许写“可评估小幅加仓”，且仍不得写成确定性指令。
...
- 缺少 `## 持仓与组合风险`，或在插件持仓存在时未披露券商覆盖/重复暴露/数据限制 -> 视为无效
- 插件持仓存在时，任一持仓缺少条件化建议标签、维持条件、失效/退出复核条件或加仓约束 -> 视为无效
